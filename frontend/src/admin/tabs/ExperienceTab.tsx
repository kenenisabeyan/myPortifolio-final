        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="bg-[#07101f] border border-white/10 border-t-[6px] border-t-cyan-500 rounded-3xl max-w-lg w-full p-6 space-y-4">
            <div className="flex justify-between items-center border-b border-white/10 pb-3">
              <h2 className="text-base font-bold text-white">{editing.id ? 'Edit Experience' : 'Add Experience'}</h2>
              <button onClick={() => setModalOpen(false)} className="text-gray-400"><FaTimes /></button>
            </div>
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="block text-xs uppercase font-semibold text-gray-300 mb-1">Job Title</label>
              ...
              <div>
                <label className="block text-xs uppercase font-semibold text-gray-300 mb-1">Company</label>
                <input type="text" required value={editing.company || ''} onChange={e => setEditing({ ...editing, company: e.target.value })} className="w-full bg-black/50 border border-white/10 rounded-xl p-2.5 text-xs text-white" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs uppercase font-semibold text-gray-300 mb-1">Responsibilities (one per line)</label>
                <textarea rows={4} value={editing.responsibilities?.join('\n') || ''} onChange={e => setEditing({ ...editing, responsibilities: e.target.value.split('\n').filter(Boolean) })} className="w-full bg-black/50 border border-white/10 rounded-xl p-2.5 text-xs text-white" />
              </div>
