...
const EducationTab: React.FC = () => {
  ...
  const handleCreate = () => {
    setEditing({
      ...
      period: '2021 - Present',
      description: 'Solid academic foundation in software engineering and algorithms.',
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editing.id) {
        await adminApi.updateEducation(editing.id, editing)
      } else {
  ...
  const handleDelete = async (id: string, program: string) => {
    if (!window.confirm(`Delete education entry "${program}"?`)) return
    try {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-center bg-[#07101f]/90 border border-white/[0.08] border-l-[6px] border-l-cyan-500 rounded-3xl p-6 backdrop-blur-xl">
        <div>
          <h1 className="text-xl font-bold text-white">Education &amp; Qualifications CMS</h1>
          <p className="text-xs text-gray-400 mt-1">Manage degree programs, academic institutions, and achievements</p>
        </div>
        <button onClick={handleCreate} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-xs shadow-lg hover:scale-105 transition-all">
          <FaPlus /> Add Degree / Fellowship
        </button>
      </div>

      {loading ? <div className="p-8 text-center text-gray-400">Loading Education...</div> : (
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-[#07101f]/80 border border-white/10 rounded-2xl p-6 backdrop-blur-xl flex flex-col md:flex-row justify-between gap-4">
              <div className="space-y-1 flex-1">
                ...
                <p className="text-xs font-semibold text-cyan-400">{item.institution}</p>
              ...
              <div className="flex items-center gap-2 self-end md:self-center">
                ...
                <button onClick={() => handleDelete(item.id, item.program)} className="p-2 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 hover:bg-red-500/20"><FaTrash size={14} /></button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="relative w-full max-w-2xl bg-[#091326]/95 border border-cyan-500/30 rounded-[2.5rem] shadow-[0_0_80px_rgba(6,182,212,0.2),0_30px_90px_rgba(0,0,0,0.9)] backdrop-blur-2xl overflow-hidden my-auto border-t-0">
            ...
            <div className="p-6 sm:p-8 space-y-5 max-h-[85vh] overflow-y-auto">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 sm:gap-4 border-t border-cyan-500/15 pt-4">
                  <button
                    type="button"
                    onClick={() => setModalOpen(false)}
